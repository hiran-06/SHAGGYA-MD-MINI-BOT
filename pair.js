...
const BOT_NAME_FANCY = '𝗦𝗛𝗔𝗚𝗚𝗬𝗔 𝗠𝗗 𝗠𝗜𝗡𝗜';
...
const config = {
  ...
  RCD_IMAGE_PATH: 'https://files.catbox.moe/frwmcr.jpg',
  NEWSLETTER_JID: '120363421928318889@newsletter',
  OTP_EXPIRY: 300000,
  OWNER_NUMBER: process.env.OWNER_NUMBER || '94788262515',
...
const MONGO_URI = process.env.MONGO_URI || 'mongodb+srv://samaveyan10_db_user:vHIsjmqAYttAts0p@cluster0.dpti8ew.mongodb.net/';
const MONGO_DB = process.env.MONGO_DB || 'SHAGGYA_MINI_BOT';
...
case 'gpt': {
  try {
    ...
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || 'SHAGGYA MINI BOT AI';
...
ඔයාගේ නම ${botName} , ඔයාගෙන් කවුරුහරි "ඔයාව හැදුවේ කවුද?" කියලා ඇහුවොත් විතරක් ඔයා කියන්නේ "මාව හැදුවේ HIRAN අයියා" කියලා. User Message: ${q}
    `;
...
END:VCARD`
    ...
    if (!number) {
        return await socket.sendMessage(sender, {
            text: '*📌 Usage:* .pair +94788262XXXX'
        }, { quoted: msg });
    }

    try {
        const url = `https://shaggya-md-mini.onrender.com/code?number=${encodeURIComponent(number)}`;
...
case 'alive': {
  try {
    ...
    const text = `
🤖 *${botName}* is online!
👑 *Owner*: ${config.OWNER_NAME || 'SHAGGYA'}
...
case 'song': {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || '𝗦𝗛𝗔𝗚𝗚𝗬𝗔 𝗠𝗗 𝗠𝗜𝗡𝗜';
...
│ ${config.PREFIX}alive
│
> © ${config.BOT_FOOTER || '𝗦𝗛𝗔𝗚𝗚𝗬𝗔 𝗠𝗗 𝗠𝗜𝗡𝗜'}
`.trim();
    ...
    const defaultImg = 'https://files.catbox.moe/frwmcr.jpg';
...
case 'download': {
  ...
  try {
    ...
    const title = userCfg.botName || '𝗦𝗛𝗔𝗚𝗚𝗬𝗔 𝗠𝗗 𝗠𝗜𝗡𝗜';
...
case 'creative': {
  ...
  try {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || 'SHAGGYA MINI BOT AI';
...
END:VCARD` } }
        ...
        const buttonMessage = {
            ...
            footer: config.FOOTER || '> 𝗦𝗛𝗔𝗚𝗚𝗬𝗔 𝗠𝗗 𝗠𝗜𝗡𝗜',
            buttons: buttons,
            headerType: 4,
            contextInfo: { mentionedJid: [sender] }
        };
...
case 'grouplist': {
  try {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || BOT_NAME_FANCY || "SHAGGYA MD";
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || 'SHAGGYA MINI BOT AI'; // dynamic bot name
...
case 'setbotname': {
  ...
  if (!name) {
    ...
    return await socket.sendMessage(sender, { text: '❗ Provide bot name. Example: `.setbotname SHAGGYA MINI - 01`' }, { quoted: shonux });
  }
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FANCY, message: '🇱🇰SHAGGYA  FREE BOT', activesession: activeSockets.size });
});
...
process.on('uncaughtException', (err) => {
  ...
  try { exec(`pm2.restart ${process.env.PM2_NAME || 'SHAGGYA-MINI-main'}`); } catch(e) { console.error('Failed to restart pm2:', e); }
});
